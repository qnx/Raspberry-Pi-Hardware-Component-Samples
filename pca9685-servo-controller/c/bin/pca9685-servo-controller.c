...
void print_help(void) {
  printf("pca9685-servo-controller [options] [pin] low high\n");
  printf("CLI PWM controller for PCA9685 device\n");
  printf("\n");
  printf("Arguments:\n");
  printf("\tpin:  Which pin is targeted for command. If no pin is given all "
         "pin are set.\n");
  printf("\tlow:  Off time for channel (0-1024)\n");
  printf("\thigh: On time for channel (0-1024)\n");
  printf("\nOptions:\n");
...
int main(int argc, char *argv[]) {
  ...
  int low = atoi(argv[optind]);
  int high = atoi(argv[optind + 1]);
  ...
  if (pin == -1) {
    printf("Setting all pins to low=%d high=%d\n", low, high);
    pwm_driver_pca9685_set_pwm_all(&driver, high, low);
  } else {
    printf("Setting pin=%d to low=%d high=%d\n", pin, low, high);
    pwm_driver_pca9685_set_pwm(&driver, pin, high, low);
  }
