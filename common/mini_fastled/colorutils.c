...
static uint8_t lsrX4( uint8_t dividend)
{
#if defined(__AVR__)
    dividend /= 2;
    dividend /= 2;
...
CRGB ColorFromPalette_ext(const CRGBPalette16 pal, uint8_t index, uint8_t brightness, TBlendType blendType)
{
    ...
    uint8_t red1 = (*entry >> LED_SHIFT_R) & 0xff;
    uint8_t green1 = (*entry >> LED_SHIFT_G) & 0xff;
    uint8_t blue1 = (*entry >> LED_SHIFT_B) & 0xff;
    ...
    if (blend)
    {
        ...
        //    rgb1.nscale8(f1);
        uint8_t red2 = (*entry >> LED_SHIFT_R) & 0xff;
        red1 = scale8_LEAVING_R1_DIRTY(red1, f1);
        red2 = scale8_LEAVING_R1_DIRTY(red2, f2);
        red1 += red2;

        uint8_t green2 = (*entry >> LED_SHIFT_G) & 0xff;
        green1 = scale8_LEAVING_R1_DIRTY(green1, f1);
        green2 = scale8_LEAVING_R1_DIRTY(green2, f2);
        green1 += green2;

        uint8_t blue2 = (*entry >> LED_SHIFT_B) & 0xff;
        blue1 = scale8_LEAVING_R1_DIRTY(blue1, f1);
